...
func genMatrix(testType string, testDirList []string) []matrixLine {
	...
	var testMatrixComplete []matrixLine
	for _, test := range testMatrix {
		for _, testDirectory := range testDirList {
			matrixLine := matrixLine{TestDir: testDirectory}
			err = mapstructure.Decode(test, &matrixLine)
			if err != nil {
				log.Panicf("can't decode map test %v to metric line struct with error %v", testDirectory, err)
			}
			matrixLine.TestType = testType
			testMatrixComplete = append(testMatrixComplete, matrixLine)
		}
	}
	return testMatrixComplete
}

func writeTestMatrixFile(testType string, testMatrix []matrixLine) {
