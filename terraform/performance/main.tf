...
resource "null_resource" "validator_windows" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "validator.exe --validator-config=${local.temp_directory}/${local.final_validator_config} --preparation-mode=true",
      local.start_command,
      "validator.exe --validator-config=${local.temp_directory}/${local.final_validator_config} --preparation-mode=false",
    ]
  }
}
