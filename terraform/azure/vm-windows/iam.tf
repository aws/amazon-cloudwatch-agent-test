...
data "aws_iam_policy_document" "cwagent_assume_role" {
  statement {
    ...
    condition {
      ...
      values   = [azurerm_windows_virtual_machine.cwagent.identity[0].principal_id]
    }
...
resource "aws_iam_role" "cwagent" {
  name               = "cwa-azurevmwin-integ-role-${module.common.testing_id}"
...
resource "aws_iam_role_policy" "cwagent" {
  name   = "cwa-azurevmwin-integ-policy-${module.common.testing_id}"
