...
resource "null_resource" "integration_test" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "go test -tags integration ${var.test_dir} -p 1 -timeout 30m -computeType=AZUREVM -region=${var.region} -cwaCommitSha=${var.cwa_github_sha} -instanceId=${azurerm_linux_virtual_machine.cwagent.virtual_machine_id} -assumeRoleArn=${aws_iam_role.cwagent.arn} -azureVMName=${azurerm_linux_virtual_machine.cwagent.name} -azureVMSize=${var.azure_vm_size} -azureResourceGroup=${var.azure_resource_group} -azureLocation=${var.azure_location} -v; test_rc=$?; rm -f /tmp/azure-identity-token; exit $test_rc",
    ]
  }
