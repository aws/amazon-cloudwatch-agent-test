...
locals {
  ...
  vendor_uri = "${var.s3_bucket}/integration-test/test-vendor/${var.cwa_github_sha}/vendor.tar.gz"
...
resource "null_resource" "integration_test_setup" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "aws s3 cp s3://${local.vendor_uri} ./vendor.tar.gz",
