...
resource "null_resource" "integration_test_run" {
  ...
  provisioner "remote-exec" {
    inline = concat(
      ...
      var.is_selinux_test ? [
        "sudo yum remove chronicle",
        "sudo systemctl stop chronicle",
        "sudo systemctl disable chronicle",
      ...
      [
        ...
        "sudo ausearch -m AVC,USER_AVC -ts recent -ts $(date --date='450 minutes ago' '+%H:%M:%S') | audit2allow -M custom_policysudo ausearch -m AVC,USER_AVC -ts recent -ts $(date --date='450 minutes ago' '+%H:%M:%S') | audit2allow -M custom_policy",
