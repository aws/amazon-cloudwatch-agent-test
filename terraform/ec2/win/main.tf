...
resource "null_resource" "integration_test_setup" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "'rules:', '- pattern: \".*\"' | Set-Content -Path \"C:\\jmx_workload\\exporter_config.yaml\"",
      "'global:', '  scrape_interval: 1m', '  scrape_timeout: 10s', 'scrape_configs:', '  - job_name: jmx-exporter', '    sample_limit: 10000', '    file_sd_configs:', '      - files: [ \"C:\\\\jmx_workload\\\\prometheus_file_sd.yaml\" ]' | Set-Content -Path \"C:\\jmx_workload\\prometheus.yaml\"",
