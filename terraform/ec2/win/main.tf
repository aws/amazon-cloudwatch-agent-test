...
resource "null_resource" "integration_test_run_validator" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "powershell.exe -Command \"$javaPath = (Get-ChildItem 'C:\\\\Program Files\\\\OpenJDK' -Directory | Sort-Object Name -Descending | Select-Object -First 1 | ForEach-Object { Join-Path $_.FullName 'bin\\\\java.exe' }); Write-Output \\\"Using Java at: $javaPath\\\"; Start-Process -FilePath $javaPath -ArgumentList \\\"-javaagent:C:\\\\jmx_workload\\\\jmx_prometheus_javaagent-0.12.0.jar=9404:C:\\\\jmx_workload\\\\exporter_config.yaml -cp C:\\\\jmx_workload\\\\SampleJavaApplication-1.0-SNAPSHOT.jar com.gubupt.sample.app.App\\\" -WindowStyle Hidden\"",
      "powershell.exe -Command \"Start-Sleep -s 30\"",
      "powershell.exe -Command \"$retries = 0; while ($retries -lt 5) { try { $response = Invoke-WebRequest -Uri http://localhost:9404/metrics -UseBasicParsing -TimeoutSec 5; if ($response.StatusCode -eq 200 -and $response.Content -match 'jvm_threads_current') { Write-Output 'JMX metrics endpoint is working and jvm_threads_current is available'; break } } catch { Write-Output 'Attempt ' + ($retries + 1) + ': JMX endpoint not ready, retrying...'; Start-Sleep -s 10; $retries++ } } if ($retries -eq 5) { throw 'JMX metrics endpoint failed to start properly' }\"",
      "set AWS_REGION=${var.region}",
