...
resource "null_resource" "integration_test_run_validator" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "if not exist amazon-cloudwatch-agent-test (echo ERROR: amazon-cloudwatch-agent-test directory not found after clone & exit 1)",
      "echo Listing cloned directory:",
      "dir amazon-cloudwatch-agent-test",
      "echo Checking for app_signals resources...",
      "if not exist amazon-cloudwatch-agent-test\\test\\app_signals\\resources\\traces\\traces.json (echo ERROR: traces.json not found & dir /s amazon-cloudwatch-agent-test\\test\\app_signals 2>nul || echo app_signals directory not found)",
      "if not exist amazon-cloudwatch-agent-test\\test\\app_signals\\resources\\metrics\\server_consumer.json (echo ERROR: server_consumer.json not found)",
      ...
      "echo Final verification before validator:",
      "if not exist C:\\Users\\Administrator\\amazon-cloudwatch-agent-test\\test\\app_signals\\resources\\traces\\traces.json (echo CRITICAL: traces.json still not found at expected path & dir /s C:\\Users\\Administrator\\amazon-cloudwatch-agent-test\\test 2>nul | findstr /i app_signals)",
      "cd C:\\Users\\Administrator\\amazon-cloudwatch-agent-test",
