...
resource "null_resource" "validator" {
  provisioner "local-exec" {
    command = <<-EOT
      ...
      cd ../../../../..
      go test ${var.test_dir} -timeout 0 -cwagentConfigSsmParamName=${local.cwagent_config_ssm_param_name} -clusterArn=${aws_ecs_cluster.cluster.arn} -cwagentECSServiceName=${aws_ecs_service.cwagent_service.name} --tags=integration
    EOT
  }
