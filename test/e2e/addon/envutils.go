...
func ApplyResources(k8ctl *utils.K8CtlManager, env *environment.MetaData) error {
	...
	if err := k8ctl.ConditionalWait("--for=condition=available", 60*time.Second,
	...
	fmt.Println("ApplyResources is complete")
	return nil
}
...
func updateImages(k8ctl *utils.K8CtlManager, env *environment.MetaData) error {
	...
	// Update CloudWatch Agent Target Allocator deployment image
	if env.CloudwatchAgentTargetAllocatorTag != "latest" {
		allocatorPatch := fmt.Sprintf(`[{
				"op": "replace",
