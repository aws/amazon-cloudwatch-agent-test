...
import (
	...
	"github.com/aws/amazon-cloudwatch-agent-test/test/e2e/addon"
)
...
func TestMain(m *testing.M) {
	...
	if env.Destroy {
		if err := addon.DestroyResources(env); err != nil {
	...
	if err := addon.InitializeEnvironment(env); err != nil {
...
func testAgentResources(t *testing.T, clientset *kubernetes.Clientset) {
	t.Run("verify_agent_resources", func(t *testing.T) {
		...
		fmt.Println("verify_agent_resources")
		addon.VerifyAgentResources(t, clientset, "jmx")
	})
}

func testJMXResources(t *testing.T, clientset *kubernetes.Clientset) {
	t.Run("verify_jmx_resources", func(t *testing.T) {
		fmt.Println("verify_jmx_resources")
		...
		addon.VerifyPodEnvironment(t, clientset, deploymentName, requiredEnvVars)
	})
}
...
func testTomcatMetrics(t *testing.T) {
	t.Run("verify_jvm_tomcat_metrics", func(t *testing.T) {
		addon.ValidateMetrics(t, []string{
			"jvm.classes.loaded",
...
func testTomcatSessions(t *testing.T) {
	t.Run("verify_tomcat_sessions", func(t *testing.T) {
		time.Sleep(addon.Wait)
		addon.VerifyMetricAboveZero(t, "tomcat.sessions", "JVM_TOMCAT_E2E")
	})
}
...
func testTomcatRejectedSessions(t *testing.T) {
	t.Run("verify_catalina_manager_rejectedsessions", func(t *testing.T) {
		time.Sleep(addon.Wait)
		addon.VerifyMetricAboveZero(t, "catalina_manager_rejectedsessions", "ContainerInsights/Prometheus")
	})
}
