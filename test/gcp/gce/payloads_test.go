...
func buildMetricsPayload(instanceID string) []byte {
  ...
  "resourceMetrics": [{
    ...
    "scopeMetrics": [{
      "scope": {"name": "gce-otlp-test-metrics", "version": "1.0.0"},
      "metrics": [
        {
          "name": "gce_otlp_counter",
          "unit": "1",
          "sum": {
...
func buildLogsPayload(instanceID string) []byte {
	now := time.Now().UnixNano()
	body := fmt.Sprintf("gce_otlp_log_%s", instanceID)
	return []byte(fmt.Sprintf(`{
  "resourceLogs": [{
    "resource": {"attributes": [
      {"key": "service.name", "value": {"stringValue": "%s"}},
...
func buildTracesPayload(instanceID string) ([]byte, string) {
  ...
  "resourceSpans": [{
    ...
    "scopeSpans": [{
      "scope": {"name": "gce-otlp-test-traces"},
      "spans": [{
        "traceId": "%s",
        "spanId": "%s",
        "name": "gce-otlp-test-span",
        "kind": 2,
        "startTimeUnixNano": "%d",
        "endTimeUnixNano": "%d",
