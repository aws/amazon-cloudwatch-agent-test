...
func TestDetailedMetricsToEMF(t *testing.T) {
	...
	// By default, OTLP -> CW Logs always goes to /aws/cwagent log group with an autogenerated log stream name, but this
	// test will set the log stream name to the instance ID.
	defer awsservice.DeleteLogStream("/aws/cwagent", instanceId)
	...
	err = modifyAgentYaml(instanceId)
...
func modifyAgentYaml(instanceId string) error {
	ampCommands := []string{
		...
		fmt.Sprintf(`sudo sed -ie 's/detailed_metrics: false/detailed_metrics: true/g;s/log_stream_name: ""/log_stream_name: "%s"/g' /opt/aws/amazon-cloudwatch-agent/etc/amazon-cloudwatch-agent.yaml`, instanceId),
	}
