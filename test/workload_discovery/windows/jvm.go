...
func testJVMCase(jarName string, manifestData map[string]string, expectedName string) error {
	// Setup JAR
	jarPath := filepath.Join("C:\\tmp", jarName)
	...
	cmd := exec.Command("powershell", "-File", "C:\\scripts.ps1", "Start-JVM", "-JarPath", jarPath)
	...
	// Test READY phase
	cmd = exec.Command("powershell", "-File", "C:\\scripts.ps1", "Start-JVM", "-JarPath", jarPath, "-Port", strconv.Itoa(JVMPort))
	output, err = cmd.Output()
	if err != nil {
